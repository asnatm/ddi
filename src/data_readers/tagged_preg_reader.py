...
class tagged_data_reader():
    ...
    def read_who(self,remove_disputed=True):
        tagged_drugs_path=os.path.join('pickles', 'data', 'preg','WHO essential med classification manually processed.xlsx') #r'C:\Users\Administrator\PycharmProjects\DDI_SMC\data\WHO essential med classification.xlsx'
    ...
    def read_smc(self,filter_sys_only=False,remove_disputed=True):
        ans = pd.read_excel(
            #os.path.join('pickles','data','preg','final classification maya.xlsx'))
            os.path.join('pickles', 'data', 'preg', 'Final list 1.2020.xlsx'))
        ans = ans.dropna(subset=['drugBank_id'])
    ...
    def read_safeFetus(self,remove_disputed=False):
        ans = pd.read_excel(
